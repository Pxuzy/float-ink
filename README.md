...
- 悬浮按钮透明度、自动隐藏和隐藏延迟设置
- 悬浮工具栏排序、启用/停用和横向滚动
- 独立黄金参考线，可显示/隐藏、同时显示水平与垂直方向、上下或左右拖动，并在黄金比例位置吸附
- 独立斐波那契回撤工具，支持可见图标、两点绘制、端点重复拖动和尺寸变化适配，显示 0/23.6/38.2/50/61.8/78.6/100% 七档水平线
- 设置修改后可实时同步到正在运行的悬浮层
- 旋转、分屏和窄屏布局适配
- 从 GitHub Releases 检查、下载并交给系统安装更新 APK
- 同一正式签名的覆盖更新不会删除应用数据；不同签名版本无法覆盖，需先导出会话并卸载旧包
